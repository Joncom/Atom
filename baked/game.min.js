...
// lib/game/entities/atom.js
ig.baked=true;ig.module('game.entities.atom').requires('impact.entity').defines(function(){EntityAtom=ig.Entity.extend({size:{x:250,y:250},children:10,init:function(x,y,settings){this.parent(x,y,settings);ig.game.spawnEntity(EntityNucleus,0,0,{container:this});for(var i=0;i<this.children;i++){ig.game.spawnEntity(EntityElectron,0,0,{container:this});}},update:function(){this.parent();}});});

// lib/game/entities/nucleus.js
ig.baked=true;ig.module('game.entities.nucleus').requires('impact.entity').defines(function(){EntityNucleus=ig.Entity.extend({size:{x:60,y:60},animSheet:new ig.AnimationSheet('media/nucleus.png',60,60),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('static',0.1,[0]);if(typeof(this.container)==='undefined'){console.debug('Killing EntityNucleus because no container was defined.');this.kill();}
this.zIndex=this.container.zIndex+1;},update:function(){this.pos.x=this.container.pos.x+(this.container.size.x/2)-(this.size.x/2);this.pos.y=this.container.pos.y+(this.container.size.y/2)-(this.size.y/2);}});});

// lib/game/entities/electron.js
ig.baked=true;ig.module('game.entities.electron').requires('impact.entity').defines(function(){EntityElectron=ig.Entity.extend({size:{x:26,y:26},speed:10,pathWidth:240,pathHeight:60,animSheet:new ig.AnimationSheet('media/electron.png',26,26),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('static',0.1,[0]);this.timer=new ig.Timer();this.timer.set(-Math.random()*10);if(typeof(this.angle)!=='number')this.angle=Math.random()*360;if(typeof(this.container)==='undefined'){console.debug('Killing EntityElectron because no container was defined.');this.kill();}},update:function(){this.parent();this.angleRadians=this.angle*(Math.PI/180);var theta=this.timer.delta()*this.speed;var x=Math.cos(theta)*(this.pathWidth/2);var y=Math.sin(theta)*(this.pathHeight/2);if(y>=0)this.zIndex=this.container.zIndex;else this.zIndex=this.container.zIndex+2;var newX=x*Math.cos(this.angleRadians)-y*Math.sin(this.angleRadians);var newY=x*Math.sin(this.angleRadians)-y*Math.cos(this.angleRadians);this.pos.x=this.container.pos.x+(this.container.size.x/2)+newX-(this.size.x/2);this.pos.y=this.container.pos.y+(this.container.size.y/2)+newY-(this.size.y/2);}});});

// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.font','game.entities.atom','game.entities.nucleus','game.entities.electron').defines(function(){MyGame=ig.Game.extend({autoSort:true,init:function(){this.atom=ig.game.spawnEntity(EntityAtom,(ig.system.width/2)-(EntityAtom.prototype.size.x/2),(ig.system.height/2)-(EntityAtom.prototype.size.y/2),{children:15});},update:function(){this.parent();},draw:function(){this.parent();}});ig.main('#canvas',MyGame,60,640,480,1);});